...
import com.rkopylknu.minimaltodo.util.REMINDER_SERVICE_NAME
...
class ReminderService : IntentService(REMINDER_SERVICE_NAME) {
    ...
    private fun deleteAlarm(toDoItem: ToDoItem) {
        ...
        StoreRetrieveData.mutate(this) {
            val index = indexOf(toDoItem)
            remove(toDoItem)
            val newToDoItem = toDoItem.copy(reminder = null)
            add(index, newToDoItem)
        }
    }
